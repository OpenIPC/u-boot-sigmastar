/*
* drvSPINAND_common_hal.c- Sigmastar
*
...
static U32 _HAL_SPINAND_BDMA_Write(U32 u32Data, U32 u32DataSize, U32 u32offset)
{
    U16 u16data;
    U32 u32Ret = ERR_SPINAND_TIMEOUT;
    ...
    BDMA_WRITE(0x03, 0x0000);   //channel0: MIU0
    // Set start address
    BDMA_WRITE(0x04, ((u32Data + u32offset) & 0x0000FFFF));
    BDMA_WRITE(0x05, (((u32Data + u32offset) >> 16)& 0x0FFF));
...
U32 HAL_SPINAND_PROGRAM_BY_BDMA(U16 u16ColumnAddr, U16 u16DataSize, U8 *pu8Data, U8 *pu8_SpareBuf)
{
    ...
    MS_U16 u16offset = 0;
    MS_U16 u16Timeout_status = 0;

    //close cs timeout avoid confict with sw cs
    u16Timeout_status = QSPI_READ(REG_SPI_TIMEOUT_H);
    if(u16Timeout_status & 0x8000)
    {
        QSPI_WRITE(REG_SPI_TIMEOUT_H, 0x4000);
    }
    ...
    while(u16DataSize)
    {
        u16writesize = u16DataSize > 2048?2048:u16DataSize;
        FSP_WRITE(REG_FSP_WBF_SIZE_OUTSIDE, (u16writesize+1)&0x0FFF);
        FSP_WRITE(REG_FSP_WBF_OUTSIDE, 0x1000);

        QSPI_WRITE(REG_SPI_BURST_WRITE,REG_SPI_ENABLE_BURST);
        FSP_WRITE(REG_FSP_WRITE_SIZE, 0x0);
        FSP_WRITE(REG_FSP_READ_SIZE, 0x0);
        FSP_WRITE(REG_FSP_CTRL2, 0x0);

        _HAL_SPINAND_BDMA_Write((U32)pu8Data, (U32)u16writesize, (U32)u16offset);
        FSP_WRITE(REG_FSP_TRIGGER, TRIGGER_FSP);
        ...
        if(bRet != ERR_SPINAND_SUCCESS)
        {
            printf("Wait for BDMA Done fails!\n");
        }

        if(_HAL_FSP_ChkWaitDone() == FALSE)
        {
            printf("PL Wait FSP Done Time Out !!!!\r\n");
            bRet = ERR_SPINAND_TIMEOUT;
            goto clean_up;
        }

        FSP_WRITE_BYTE(REG_FSP_CLEAR_DONE, CLEAR_DONE_FSP);
        u16DataSize -= u16writesize;
        u16offset += u16writesize;
    }
...
clean_up:
    FSP_WRITE(REG_FSP_WBF_SIZE_OUTSIDE, 0x0);
    FSP_WRITE(REG_FSP_WBF_OUTSIDE, 0x0);
    QSPI_WRITE(REG_SPI_BURST_WRITE,0x2);
    FSP_WRITE_BYTE(REG_FSP_CLEAR_DONE, CLEAR_DONE_FSP);
    //resume timeout status
    if(u16Timeout_status & 0x8000)
    {
        QSPI_WRITE(REG_SPI_TIMEOUT_H, u16Timeout_status);
    }
    return bRet;
}
