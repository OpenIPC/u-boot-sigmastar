...
U32  eMMCTest_SgW_MIU(U32 u32_eMMC_Addr)
{
    ...
    u32_dmaaddr = eMMC_translate_DMA_address_Ex((U32)p_AdmaDesc_st, sizeof(p_AdmaDesc_st)*MAX_SCATTERLIST_COUNT, WRITE_TO_eMMC);
...
U32 eMMCTest_MultiBlkWRC_MIU(U32 u32_eMMC_Addr, U16 u16_BlkCnt, U32 u32_DataPattern)
{
    ...
    for(u32_i=0; u32_i<u16_BlkCnt; u32_i++)
    {
        u16_BlkCnt_tmp = (u16_BlkCnt_tmp + u32_i) > u16_BlkCnt ? (u16_BlkCnt - u32_i) : u16_BlkCnt_tmp;

        u32_err = eMMC_CMD25_MIU(
            u32_eMMC_Addr+u32_i, gau8_WBuf+(u32_i<<eMMC_SECTOR_512BYTE_BITS), u16_BlkCnt_tmp);
        if(eMMC_ST_SUCCESS != u32_err)
        {
        ...
        u32_err = eMMC_CMD18_MIU(
            u32_eMMC_Addr+u32_i, gau8_RBuf+(u32_i<<eMMC_SECTOR_512BYTE_BITS), u16_BlkCnt_tmp);
        if(eMMC_ST_SUCCESS != u32_err)
        {
        ...
        u32_err = eMMC_ComapreData(
            gau8_WBuf+(u32_i<<eMMC_SECTOR_512BYTE_BITS), gau8_RBuf+(u32_i<<eMMC_SECTOR_512BYTE_BITS), u16_BlkCnt_tmp<<eMMC_SECTOR_512BYTE_BITS);
        if(eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(0,1,"Err, MultiBlkWRC C, %u, %Xh \n", u32_i, u32_err);
            goto LABEL_OF_ERROR;
