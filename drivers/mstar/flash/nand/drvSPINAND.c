...
static u8 drv_spinand_read_from_cache(u16 u16_col_address, u8 *pu8_data, u32 u32_size)
{
    ...
    if (0 != ((unsigned long)pu8_data % 64) || 0 != (u32_size % 64) || 0 != (u16_col_address % 16))
    {
...
static u8 drv_spinand_read_from_cache(u16 u16_col_address, u8 *pu8_data, u32 u32_size)
{
    ...
    if (0 != ((unsigned long)pu8_data % 16) || 0 != (u16_col_address % 8) || u32_size < 64)
    {
...
static u8 drv_spinand_complete_read_from_cache(u32 u32_page, u16 u16_column, u8 *pu8_data, u32 u32_size)
{
    if (ERR_SPINAND_SUCCESS
        != drv_spinand_read_from_cache(u16_column | drv_spinand_offset_wrap(u32_page, SPINAND_READ), pu8_data, u32_size))
    {
...
static u8 drv_spinand_complete_random_program_load(u16 u16_col_address, u8 *pu8_data, u32 u32_size)
{
    u8  u8_status;
    u32 u32_cmd;
    ...
    {
        DRV_FSP_set_access_mode(drv_spinand_get_cmd_mode(st_spinand_drv_info.u8_randomLoad));
...
static u8 drv_spinand_complete_program_load(u16 u16_col_address, u8 *pu8_data, u32 u32_size)
{
    u8  u8_status;
    u32 u32_cmd;
    ...
    if (ERR_SPINAND_SUCCESS == (u8_status = drv_spinand_simple_transmission((u8 *)&u32_cmd, 3)))
    {
        DRV_FSP_set_access_mode(drv_spinand_get_cmd_mode(st_spinand_drv_info.u8_programLoad));
        u8_status = drv_spinand_load_data_to_cache(pu8_data, u32_size);
        DRV_FSP_set_access_mode(FSP_SINGLE_MODE);
    }
...
static u32 drv_spinand_read_pages_without_buf_mode(u32 u32_page, u8 *pu8_data, u32 u32_size)
{
    ...
    while (0 != u32_bytes_load)
    {
        if (0 != (u32_bytes_load & ~NAND_PAGE_SIZE_MASK))
        {
            do
            {
                u32_bytes_load = (u32_size & ~NAND_PAGE_SIZE_MASK);
                if (ERR_SPINAND_ECC_NOT_CORRECTED <= drv_spinand_page_read_status(u32_page))
                {
                    break;
                }

                drv_spinand_complete_read_from_cache(u32_page, 0, pu8_data, u32_bytes_load);
                ...
                u32_page += drv_spinand_offset_to_page_address(u32_bytes_load);
                pu8_data += u32_bytes_load;
                u32_bytes_load = u32_size - u32_bytes_load;
            } while (0);

            if (NAND_PAGE_SIZE < u32_bytes_load)
            {
                break;
            }
        }
        ...
        {
            if (u32_bytes_load != drv_spinand_read_pages(u32_page, pu8_data, u32_bytes_load))
            {
                break;
            }
...
static u8 drv_spinand_write_back_sni(u8 *sni_buf)
{
    ...
    pst_match_sni      = (SPINAND_SNI_t *)sni_buf;
    ...
    do
    {
        ...
        if (u32_page & FLASH_PAGES_PER_BLOCK_DEFAULT)
        {
            ...
            if (ERR_SPINAND_TIMEOUT <= mdrv_spinand_page_read(u32_search_page, 0, sni_buf, FLASH_SNI_HEADER_SIZE))
                continue;
    ...
    while (1)
    {
        if (!u8_ismatch)
        {
            for (; 0 != (u32_search_page & FLASH_BLOCK_MASK_DEFAULT); u32_search_page++)
            {
                ...
                {
                    if (ERR_SPINAND_SUCCESS
                        != mdrv_spinand_page_read(u32_search_page, u16_bytes, sni_buf, FLASH_MAX_SNI_SIZE))
                    {
                        return 0;
                    }
                    ...
                    if (pst_match_sni->u32_crc32
                        != FLASH_IMPL_CRC32(0, (void *)sni_buf,
                                            (unsigned long)&pst_match_sni->u32_crc32 - (unsigned long)sni_buf))
                        break;
...
void mdrv_spinand_socecc_init(u8 *imi_cache_buf, u8 *miu_cache_buf)
{
    ...
    st_fcie_cfg.random_program_load  = drv_spinand_complete_random_program_load;
...
static void drv_spinand_setup_by_sni(SPINAND_EXT_INFO_t *pst_ext_sni)
{
    ...
    if (SPINAND_NONE_BUF_MODE == pst_access->st_crMode.u8_crType)
    {
        drv_spinand_read = drv_spinand_read_pages_without_buf_mode;
        FLASH_IMPL_PRINTF("[FLASH] Load without buffer mode\r\n");
    }
    else if (SPINAND_BUF_MODE == pst_access->st_crMode.u8_crType)
    {
...
u8 mdrv_spinand_find_sni_form_dram(u8 *sni_buf, u8 *sni_list)
{
    ...
    while (drv_spinand_is_sni_header(sni_list))
    {
                                       ...
                                       (unsigned long)&pst_sni->u32_crc32 - (unsigned long)sni_list))
        {
            ...
            if (drv_spinand_soc_ecc_is_enable())
            {
                if ((pst_info->u16_PageByteCnt == ast_flash_config_list[g_socecc_type].u16_pageSize) && (pst_info->u16_SpareByteCnt >= ast_flash_config_list[g_socecc_type].u16_spareSize))
                    pst_sni->spinand_ext_info.st_profile.st_eccConfig.u8_eccType = g_socecc_type;
                else if ((pst_info->u16_PageByteCnt == ast_flash_config_list[0].u16_pageSize) && (pst_info->u16_SpareByteCnt >= ast_flash_config_list[0].u16_spareSize))
                    pst_sni->spinand_ext_info.st_profile.st_eccConfig.u8_eccType = 0;
                else if ((pst_info->u16_PageByteCnt == ast_flash_config_list[3].u16_pageSize) && (pst_info->u16_SpareByteCnt >= ast_flash_config_list[3].u16_spareSize))
...
u8 mdrv_spinand_page_program(u32 u32_page, u16 u16_column, u8 *pu8_data, u32 u32_size)
{
    ...
    {
        if (ERR_SPINAND_SUCCESS
            != drv_spinand_complete_program_load(u16_column | drv_spinand_offset_wrap(u32_page, SPINAND_PROGRAM), pu8_data, u32_size))
        {
...
u8 mdrv_spinand_cache_program(u32 u32_page, u16 u16_column, u8 *pu8_data, u32 u32_size)
{
    ...
    drv_spinand_wait_device_available();
    if (ERR_SPINAND_SUCCESS
        != drv_spinand_complete_random_program_load(u16_column | drv_spinand_offset_wrap(u32_page, SPINAND_RANDOM), pu8_data, u32_size))
    {
