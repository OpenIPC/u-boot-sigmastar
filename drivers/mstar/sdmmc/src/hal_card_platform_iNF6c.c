/*
* hal_card_platform_iNF6c.c- Sigmastar
*
...
void Hal_CARD_IPOnceSetting(IPEmType eIP)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,REG_CLK_SD0), BIT06_T|BIT05_T|BIT04_T|BIT03_T|BIT02_T|BIT01_T|BIT00_T); //[6]boot_Sel [5:2]: Clk_Sel [1]: Clk_i [0]: Clk_g
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,REG_CLK_SD0), BIT06_T);
        //Hal_SDMMC_SetSDIODevice(eIP, false);
    }
    else if(eIP == EV_IP_FCIE2)
    {
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_SC_GP_CTRL_BANK,0x25), BIT05_T); //[B3/B7]:[SDIO/SD] select BOOT clock source (glitch free) - 0: select BOOT clock 12MHz (xtali), 1: select FCIE/SPI clock source
        //Enable Clock Source to avoid reset fail
...
void Hal_CARD_InitPADPin(PADEmType ePAD, BOOL_T bTwoCard)
{
    ...
    if(ePAD == EV_PAD1) //PAD_SDIO
    {
        ...
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x06), BIT04_T);

        //reg_sd0_drv: CLK, D3, D2, D1, D0, CMD
    ...
    else if(ePAD == EV_PAD2) //PAD_FCIE
    {
        ...
        //reg_sd0_ps:CDZ=> pull up
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x16), BIT04_T);
...
void Hal_CARD_SetPADToPortPath(IPEmType eIP, PortEmType ePort, PADEmType ePAD, BOOL_T bTwoCard)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        if(ePAD == EV_PAD1)  //PAD_SD
        {
            ...
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT08_T | BIT11_T); //[B8:B11]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
    }
    else if(eIP == EV_IP_FCIE2)
    {
        if(ePAD == EV_PAD2)  //PAD_FCIE
        {
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x61), BIT09_T | BIT08_T | BIT04_T | BIT00_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x61), BIT04_T | BIT00_T); //[B0:B4]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
...
void Hal_CARD_PullPADPin(PADEmType ePAD, PinPullEmType ePinPull, BOOL_T bTwoCard)
{
    if(ePAD == EV_PAD1) //PAD_SD
    {
        if(ePinPull ==EV_PULLDOWN)
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT10_T | BIT09_T | BIT08_T);   //[B8:B10]reg_sd_mode
        ...
        else if(ePinPull == EV_PULLUP)
        {
            ...
            //SD_CLK
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x02), BIT02_T); // reg_sd0_gpio_oen_4

            //SD_CMD
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x03), BIT02_T); // reg_sd0_gpio_oen_5

            //SD_D0
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x01), BIT02_T); // reg_sd0_gpio_oen_3

            //SD_D1
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x00), BIT02_T); // reg_sd0_gpio_oen_2

            //SD_D2
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x05), BIT02_T); // reg_sd0_gpio_oen_7

            //SD_D3
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x04), BIT02_T); // reg_sd0_gpio_oen_6

            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT08_T); //[B8:B10]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
    }
    else if(ePAD == EV_PAD2) //PAD_FCIE
    {
        if (ePinPull == EV_PULLDOWN)
        {
            ...
            //SD_D0
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x11), BIT02_T);           // output mode
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x11), BIT01_T);           // output:0
        ...
        else if(ePinPull == EV_PULLUP)
        {
            ...
            //SD_D0
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x11), BIT02_T); // input mode
            //SD_D1
...
void Hal_CARD_SetClock(IPEmType eIP, U32_T u32ClkFromIPSet)
{
    ...
    else if(eIP == EV_IP_FCIE2)
    {
        ...
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_SC_GP_CTRL_BANK,0x25), BIT07_T | BIT05_T); //[B3/B7]:[SDIO/SD] select BOOT clock source (glitch free) - 0: select BOOT clock 12MHz (xtali), 1: select FCIE/SPI clock source
...
void Hal_CARD_PowerOn(PADEmType ePAD, U16_T u16DelayMs)
{
    if(ePAD==EV_PAD1) //PAD_SD
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT02_T); //reg_gpio_oen_34
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT01_T); //reg_gpio_out_34
    }
    else if(ePAD==EV_PAD2) //PAD_NAND
    {
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT03_T);
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT02_T); //reg_gpio_oen_52
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT01_T); //reg_gpio_out_52
    }
...
void Hal_CARD_PowerOff(PADEmType ePAD, U16_T u16DelayMs)
{
    if( (ePAD==EV_PAD1)) //PAD_SD
    {
        ...
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT03_T);
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT02_T); // reg_gpio_oen_34
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT01_T); // reg_gpio_out_34
    }
    else if(ePAD==EV_PAD2) //PAD_NAND
    {
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT03_T);
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT02_T);   // output mode enable
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT01_T);   // output high
    }
...
void Hal_CARD_InitGPIO(GPIOEmType eGPIO, BOOL_T bEnable)
{
    ...
    {
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x06), BIT02_T);  //PAD_SD0_CDZ:reg_sd0_gpio_oen_1
    }
    else if( eGPIO==EV_GPIO2 ) //EV_GPIO2 for Slot 1
    {
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x16), BIT02_T);
    }
...
BOOL_T Hal_CARD_GetGPIOState(GPIOEmType eGPIO)
{
    ...
    {
        #if (EN_SDMMC_CDZREV)
        u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x06)) & BIT00_T; //PAD_SD0_CDZ:reg_sd0_gpio_in_1
        #else
        u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x06)) & BIT00_T;
        #endif
    ...
    {
        #if (EN_SDMMC_CDZREV)
        u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x16)) & BIT00_T;
        #else
        u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x16)) & BIT00_T;
        #endif
        if(!u16Reg) //Low Active
