...
static BOOL_T gb_En_CLK_Phase[3]  = {FALSE, FALSE, FALSE};
static U32_T gu32_TX_CLK_Phase[3] = {0, 0, 0};
static U32_T gu32_RX_CLK_Phase[3] = {0, 0, 0};
...
#define REG_CLK_SDIO0 (0x45) //org
    ...
    #define FCIE1_CLK REG_CLK_SDIO0
        ...
        if (gb_En_CLK_Phase[eIP])
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,FCIE1_CLK), BIT14_T|BIT13_T|BIT12_T|BIT11_T|BIT10_T|BIT09_T|BIT08_T|BIT07_T);
        }
        ...
        switch(u32ClkFromIPSet)
        {
            case CLK1_F:      //48000KHz
                if (gb_En_CLK_Phase[eIP])
                {
                    ...
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, FCIE1_CLK), BIT05_T | BIT08_T | BIT13_T | (gu32_TX_CLK_Phase[eIP]<<9) | (gu32_RX_CLK_Phase[eIP]<<11));
                }
                break;
    ...
    else if(eIP == EV_IP_FCIE2)
    {
        ...
        if (gb_En_CLK_Phase[eIP])
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,FCIE2_CLK), BIT14_T|BIT13_T|BIT12_T|BIT11_T|BIT10_T|BIT09_T|BIT08_T|BIT07_T);
        }
        ...
        switch(u32ClkFromIPSet)
        {
            case CLK2_F:      //48000KHz
                if (gb_En_CLK_Phase[eIP])
                {
                    ...
                    //[b5]:select clk_spi_synth_pll, [b8]:enable tx phase [b13]:enable rx phase
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, FCIE2_CLK), BIT05_T | BIT08_T | BIT13_T | (gu32_TX_CLK_Phase[eIP]<<9) | (gu32_RX_CLK_Phase[eIP]<<11));
                }
                break;
