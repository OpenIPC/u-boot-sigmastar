...
U32 eMMC_USER_Set_WriteProtect(U32 dataAddress, U32 u32_size)
{
    U32 u32_err;
    U32 u32_start;
    ...
    // check if u32_DataByteCnt is 512B boundary
    if(dataAddress + (u32_size>>9) > g_eMMCDrv.u32_SEC_COUNT)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,1,"eMMC Err: invalid data range, %Xh > %Xh \n",
            dataAddress + (u32_size>>9), g_eMMCDrv.u32_SEC_COUNT);
        return eMMC_ST_ERR_INVALID_PARAM;
    }

    if(g_eMMCDrv.u8_ECSD171_UserWP & (BIT2|BIT0))
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,1,"eMMC is not in tmp WP mode (%X)\n", g_eMMCDrv.u8_ECSD171_UserWP & (BIT2|BIT0));
        return -1;
    }

    u32_start = ((dataAddress/g_eMMCDrv.u32_WP_group_size)*g_eMMCDrv.u32_WP_group_size);
    while(u32_start <= (dataAddress+u32_size -1))
    {
        u32_err = eMMC_CMD28_CMD29(u32_start, 28);
        if(u32_err)
            return u32_err;

        u32_start += g_eMMCDrv.u32_WP_group_size;
    }

    return u32_err;
}

// clear the target memory in the user partition from the write-protected
U32 eMMC_USER_Clr_WriteProtect(U32 dataAddress, U32 u32_size)
{
    U32 u32_err;
    U32 u32_start;
    ...
    while(u32_start <= (dataAddress+u32_size -1))
    {
        u32_err = eMMC_CMD28_CMD29(u32_start, 29);
        if(u32_err)
            return u32_err;
...
// Get whether the target memory in the user partition is write-protected
U32 eMMC_USER_Get_WriteProtect(U32 dataAddress)
{
    U32 u32_err;

    memset(gau8_eMMC_SectorBuf, 0, 0x200);

    u32_err = eMMC_CMD30_CMD31_MIU(dataAddress, gau8_eMMC_SectorBuf, 30);
    if (u32_err)
        return -1;

    return (gau8_eMMC_SectorBuf[3] & BIT0);
}

// Get the write protection type of the target memory in the user partition
U32 eMMC_USER_Get_WriteProtect_Type(U32 dataAddress)
{
    U32 u32_err;

    memset(gau8_eMMC_SectorBuf, 0, 0x200);

    u32_err = eMMC_CMD30_CMD31_MIU(dataAddress, gau8_eMMC_SectorBuf, 31);
    if (u32_err)
        return -1;

    return (gau8_eMMC_SectorBuf[7] & 0x03);
}
