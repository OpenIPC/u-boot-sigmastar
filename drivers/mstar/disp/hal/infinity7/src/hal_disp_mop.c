...
u8 g_au8MopYDmaThreshold[E_HAL_DISP_MOP_NUM] = {HAL_DISP_MOP_Y_DMA_THRESHOLD , HAL_DISP_MOP_Y_DMA_THRESHOLD, HAL_DISP_MOP_Y_DMA_THRESHOLD};
u8 g_au8MopCDmaThreshold[E_HAL_DISP_MOP_NUM] = {HAL_DISP_MOP_C_DMA_THRESHOLD , HAL_DISP_MOP_C_DMA_THRESHOLD, HAL_DISP_MOP_C_DMA_THRESHOLD};
//-------------------------------------------------------------------------------------------------
//  Local Functions
//-------------------------------------------------------------------------------------------------
...
void HalDispMopgInit(void *pCtx)
{
    ...
    for (eMopId = E_HAL_DISP_MOPID_00; eMopId < E_HAL_DISP_MOP_NUM; eMopId++)
    {
        ...
        HalDispMopgSetDmaYFifoThrd(pCtx, eMopId, g_au8MopYDmaThreshold[eMopId], 1); // h04 [7:0]ymd_thd
        HalDispMopgSetDmaCFifoThrd(pCtx, eMopId, g_au8MopCDmaThreshold[eMopId], 1); // h05 [7:0]cdma_thd; default=0xf8, use 0xd0
...
void HalDispMopsInit(void *pCtx)
{
    ...
    for (eMopId = E_HAL_DISP_MOPID_00; eMopId < E_HAL_DISP_MOP_NUM; eMopId++)
    {
        ...
        HalDispMopsSetDmaYFifoThrd(pCtx, eMopId, g_au8MopYDmaThreshold[eMopId], 1); // h04 [7:0]ymd_thd
        HalDispMopsSetDmaCFifoThrd(pCtx, eMopId, g_au8MopCDmaThreshold[eMopId], 1); // h05 [7:0]cdma_thd; default=0xf8, use 0xd0
...
void HalDispMopsSetYAddr(void *pCtx, HalDispMopId_e eMopId, ss_phys_addr_t paPhyAddr)
{
    ...
    u32MopsGwinRegExtBase = (eMopId == E_HAL_DISP_MOPID_01)   ? REG_DISP_MOPS_BK1_3F_L
                            : (eMopId == E_HAL_DISP_MOPID_02) ? REG_DISP_MOPS_BK2_3F_L
                                                              : REG_DISP_MOPS_BK0_3F_L;

    u32RegVal  = (u32)(paPhyAddr >> HAL_DISP_MOP_MIU_BUS);
    u16ExtAddr = (u16)(u32RegVal >> 24);
    if (pCmdqCtx)
    {
        ...
        if (u16ExtAddr)
        {
            HalDispUtilityW2BYTEMSK(u32MopsGwinRegExtBase, u16ExtAddr + 1, 0x000F, pCmdqCtx);
        }
        else
        {
            HalDispUtilityW2BYTEMSK(u32MopsGwinRegExtBase, 0, 0x000F, pCmdqCtx);
        }
    }
}
...
void HalDispMopsSetCAddr(void *pCtx, HalDispMopId_e eMopId, ss_phys_addr_t paPhyAddr)
{
    ...
    if (pCmdqCtx)
    {
        ...
        if (u16ExtAddr)
        {
            HalDispUtilityW2BYTEMSK(u32MopsGwinRegExtBase, (u16ExtAddr + 1) << 4, 0x00F0, pCmdqCtx);
        }
        else
        {
            HalDispUtilityW2BYTEMSK(u32MopsGwinRegExtBase, 0, 0x00F0, pCmdqCtx);
        }
    }
}
...
void HalDispMopSetYDmaThreshold(HalDispMopId_e enMopId, u8 u8Threshold)
{
    g_au8MopYDmaThreshold[enMopId] = u8Threshold;
}

void HalDispMopSetCDmaThreshold(HalDispMopId_e enMopId, u8 u8Threshold)
{
    g_au8MopCDmaThreshold[enMopId] = u8Threshold;
}
