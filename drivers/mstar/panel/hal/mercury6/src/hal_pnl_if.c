...
{
    if((pPnlCtx->pstHalCtx->stParamCfg.eLinkType == E_HAL_PNL_LINK_TTL)||(pPnlCtx->pstHalCtx->stParamCfg.eLinkType == E_HAL_PNL_LINK_TTL_SPI_IF))
    {
...
void __HalPnlIfSetPadMux(HalPnlLinkType_e eLinkType)
{
    switch (eLinkType)
    {
        ...
        case E_HAL_PNL_LINK_TTL_SPI_IF:
...
void __HalPnlIfSetUnifiedTimeGenConfig(DrvPnlCtxConfig_t *pPnlCtx, HalPnlUnifiedTgnTimingConfig_t *pstTimingCfg)
{
    ...
    if(pPnlCtx->pstHalUnfdCtx->stHalPnlUdParamCfg.eLinkType != E_HAL_PNL_LINK_MIPI_DSI)
    {
        ...
        HalPnlSetLcdRgbHActive(pstHwTimeGenCfg->u16Hactive);
        HalPnlSetLcdRgbHSyncWidth(pstHwTimeGenCfg->u16HsyncWidth - 1);
        if (pPnlCtx->pstHalUnfdCtx->stHalPnlUdParamCfg.eLinkType == E_HAL_PNL_LINK_BT656 ||
            pPnlCtx->pstHalUnfdCtx->stHalPnlUdParamCfg.eLinkType == E_HAL_PNL_LINK_BT1120)
        {
            HalPnlSetLcdRgbHBp(pstHwTimeGenCfg->u16HTotal - pstHwTimeGenCfg->u16Hactive);
        }
        else
        {
            HalPnlSetLcdRgbHBp(pstHwTimeGenCfg->u16HsyncWidth + pstHwTimeGenCfg->u16HsyncBackPorch);
...
void __HalPnlIfSetUnifiedPrototypeConfig(DrvPnlCtxConfig_t *pPnlCtx)
{
    ...
    HalPnlRgbDataType_e eRgbDtype = pPnlCtx->pstHalUnfdCtx->stHalPnlUdParamCfg.stRgbDataInfo.eRgbDtype;
    u8                  u8RgbDswap = pPnlCtx->pstHalUnfdCtx->stHalPnlUdParamCfg.stRgbDataInfo.u8RgbDswap;

    HalPnlSetLcdRgbDswap(u8RgbDswap);

    if((eLinkType == E_HAL_PNL_LINK_TTL)||(eLinkType == E_HAL_PNL_LINK_TTL_SPI_IF))
    {
        HalPnlSetLcdDisp1Used(1);
        HalPnlSetLcdDisp1Type(2);
