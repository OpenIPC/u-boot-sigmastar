...
static int do_spi_flash_read_write(int argc, char * const argv[])
{
    ...
    else
    {
        ...
        if (argv[4])
        {
            partition_offset = simple_strtoul(argv[4], &endp, 16);
            if (partition_offset + len > maxsize)
            {
                puts("Size exceeds partition or device limit\n");
                return -1;
            }
...
#if defined(CONFIG_MS_NOR_ONEBIN)
            ret = spi_flash_write(flash, offset + partition_offset, len, buf);
#else
