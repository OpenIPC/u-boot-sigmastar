...
int nor_flash_get_partitioninfo(char *partition, void *pPartitionInfo)
{
    mxp_record rec;
    FS_PARTITION_Info_t * lfsPartInfo = NULL;
    if(NULL == partition || NULL == pPartitionInfo)
    {
        return -1;
    }
    lfsPartInfo = (FS_PARTITION_Info_t*)pPartitionInfo;
    if(!mxp_get_record_by_name(partition, &rec))
    {
        lfsPartInfo->u32_PageSize = norflash->page_size;
        if(!lfsPartInfo->u8_UseExternBlockSize)
        {
            lfsPartInfo->u32_BlkSize = norflash->sector_size;
        }
        lfsPartInfo->u16_StartBlk = ((unsigned int)rec.start)/lfsPartInfo->u32_BlkSize;
        lfsPartInfo->u16_BlkCnt = ((unsigned int)rec.size)/lfsPartInfo->u32_BlkSize;
    }
