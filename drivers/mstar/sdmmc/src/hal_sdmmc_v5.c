...
static U16_T _REG_GetMIEFunCtlSetting(IPEmType eIP)
{
	...
	return (ePort==EV_PFCIE5_FCIE ? R_EMMC_EN : R_SDIO_MODE);
}
...
void Hal_SDMMC_SetBusTiming(IPEmType eIP, BusTimingEmType eBusTiming)
{
	...
	switch ( eBusTiming )
	{
		...
		case EV_BUS_DEF:
		case EV_BUS_HS:
#if ((D_PROJECT == D_PROJECT__iNF6C) || (D_PROJECT == D_PROJECT__M6P) || (D_PROJECT == D_PROJECT__iNF7))
			gu16_DDR_MODE_REG[eIP] = (R_PAD_CLK_SEL|R_PAD_IN_SEL);
#else
