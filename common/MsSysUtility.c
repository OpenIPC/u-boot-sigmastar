...
int do_gpio( cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
    ...
    gpio_num_p1 = (U8)simple_strtoul(argv[2], NULL, 10);

    if((NULL == strcmp(argv[1], "get")) || (NULL == strncmp(argv[1], "inp", 3)))
    {
        MDrv_GPIO_Pad_Set(gpio_num_p1);
        MDrv_GPIO_Pad_Odn(gpio_num_p1);
        UBOOT_INFO("IN  ");
        MDrv_GPIO_Pad_Read(gpio_num_p1, &gpio_level);
        UBOOT_INFO("pin=%d\n", gpio_level);
    }
    ...
    {
        MDrv_GPIO_Pad_InOut(gpio_num_p1, &gpio_direction);
        if(gpio_direction)
        {
            UBOOT_INFO("IN  ");
        }
    ...
    else if(NULL == strncmp(argv[1], "list", 3))
    {
        ...
        {
            size = (U8)simple_strtoul(argv[2], NULL, 10);
        }
