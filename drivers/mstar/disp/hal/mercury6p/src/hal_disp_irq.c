...
void HalDispIrqEnableDoubleVsync(u8 u8bEn, void *pCtx)
{
    HalDispUtilityCmdqContext_t *pCmdqCtx = NULL;

    HalDispGetCmdqByCtx(pCtx, (void *)&pCmdqCtx);
    HalDispUtilityW2BYTEMSK(DISP_SGWDMA_REG_WEVENT_CLR, u8bEn ? HAL_DISP_IRQ_DMAMISC_DOUBLEVS_MSK : 0,
                            HAL_DISP_IRQ_DMAMISC_DOUBLEVS_MSK, pCmdqCtx);
    HalDispUtilityW2BYTEMSK(DISP_SGWDMA_REG_WEVENT_MASK, u8bEn ? 0 : HAL_DISP_IRQ_DMAMISC_DOUBLEVS_MSK,
                            HAL_DISP_IRQ_DMAMISC_DOUBLEVS_MSK, pCmdqCtx);
}
static __inline void _HalDispIrqEnableDmaIsr(HalDispIrqType_e enType, u8 *pbEn, u32 u32DmaId)
{
    ...
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_DONE)
    {
        u16Val |= bEnable ? 0 : HAL_DISP_IRQ_DMA_BLANKING_BIT;
        u16Msk |= HAL_DISP_IRQ_DMA_BLANKING_MSK;
    }
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_ACTIVE_ON)
    {
        u16Val |= bEnable ? 0 : HAL_DISP_IRQ_DMA_VSYNC_BIT;
        u16Msk |= HAL_DISP_IRQ_DMA_VSYNC_MSK;
    }
    ...
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_MISC)
    {
        u16Val |= bEnable ? 0 : HAL_DISP_IRQ_DMA_MISC_BIT;
        u16Msk |= HAL_DISP_IRQ_DMA_MISC_MSK;
        if (bEnable)
        {
            HalDispUtilityW2BYTEMSKDirectAndNoshadow(DISP_SGWDMA_REG_WEVENT_CLR, E_HAL_DISP_IRQ_MSK_DMA_MISC,
                                                     mask_of_disp_sgwdma_reg_wevent_clr); // for MISC Y ,enable Y only
            HalDispUtilityW2BYTEMSKDirectAndNoshadow(DISP_WDMA_REG_SC_FRM_W_INT_CLR,
                                                     mask_of_disp_wdma_reg_sc_frm_w_int_clr,
                                                     mask_of_disp_wdma_reg_sc_frm_w_int_clr); // for misc Y/U/V
            HalDispUtilityW2BYTEMSKDirect(DISP_SGWDMA_REG_WEVENT_MASK, mask_of_disp_sgwdma_reg_wevent_mask,
                                          mask_of_disp_sgwdma_reg_wevent_mask, NULL);
            HalDispUtilityW2BYTEMSKDirect(DISP_WDMA_REG_SC_FRM_W_INT_MASK, mask_of_disp_wdma_reg_sc_frm_w_int_mask,
                                          mask_of_disp_wdma_reg_sc_frm_w_int_mask, NULL);
        }
        HalDispUtilityW2BYTEMSKDirect(DISP_SGWDMA_REG_WEVENT_MASK, bEnable ? 0 : E_HAL_DISP_IRQ_MSK_DMA_MISC,
                                      E_HAL_DISP_IRQ_MSK_DMA_MISC, NULL);
        HalDispUtilityW2BYTEMSKDirect(DISP_WDMA_REG_SC_FRM_W_INT_MASK, bEnable ? 0 : E_HAL_DISP_IRQ_MSK_WDMA_YUV_MISC,
                                      E_HAL_DISP_IRQ_MSK_WDMA_YUV_MISC, NULL);
    }
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_TRIG_FAIL)
    {
        u16Val |= bEnable ? 0 : HAL_DISP_IRQ_DMA_TRIG_FAIL_BIT;
        u16Msk |= HAL_DISP_IRQ_DMA_TRIG_FAIL_MSK;
    }
    if (bEnable)
    {
...
void HalDispIrqEnableVsync(u8 u8bEn, u32 u32DmaId, void *pCtx)
{
    ...
    if (u8bEn)
    {
        HalDispUtilityW2BYTEMSK(HAL_DISP_IRQ_DMA_CLEAR_REG(u32DmaId), HAL_DISP_IRQ_DMA_VSYNC_BIT,
                                HAL_DISP_IRQ_DMA_VSYNC_MSK, pCmdqCtx);
    }
    HalDispUtilityW2BYTEMSK(HAL_DISP_IRQ_DMA_MASK_REG(u32DmaId), u8bEn ? 0 : HAL_DISP_IRQ_DMA_VSYNC_BIT,
                            HAL_DISP_IRQ_DMA_VSYNC_MSK, pCmdqCtx);
}
static void _HalDispIrqDmaDoubleVsyncHandler(u64 u64Time, u32 u32DmaId)
{
    ...
    DISP_STATISTIC_VAL(u64DoneTime = g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u64TimeStamp[E_HAL_DISP_IRQ_DMA_REALDONE]);
    if (u64Time - u64DoneTime > 1000)
    {
        DISP_ERR("%s %d, Id:%d, Double Vsync without frame done VCnt:%hu,FDT@:%llu ,Rst @%llu\n", __FUNCTION__,
...
static void _HalDispIrqDmaMiscHandler(u64 u64Time, u32 u32DmaId)
{
    // only check Y
    u16 u16Val, u16Msk;
    u16 u16ClrVal = 0;

    u16Val    = (R2BYTE(DISP_SGWDMA_REG_WEVENT_STATUS) >> shift_of_disp_sgwdma_reg_wevent_status);
    u16Msk    = (R2BYTE(DISP_SGWDMA_REG_WEVENT_MASK) >> shift_of_disp_sgwdma_reg_wevent_mask);
    u16ClrVal = u16Val & (~u16Msk);
    DISP_DBG(DISP_DBG_LEVEL_IRQ_DMA, "%s %d, Id:%d,  Msk:%04x, Val:%04x \n", __FUNCTION__, __LINE__, u32DmaId, u16Msk,
             u16Val);
    if (u16ClrVal & HAL_DISP_IRQ_DMAMISC_DOUBLEVS_BIT)
    {
        g_u8DvContinue++;
    ...
    if (u16ClrVal & HAL_DISP_IRQ_DMAMISC_OVERWRITE_BIT)
    {
        DISP_STATISTIC_VAL(g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u32OverWriteCnt++);
        DISP_STATISTIC_VAL(g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u64TimeStamp[E_HAL_DISP_IRQ_DMA_OVERWRITE] = u64Time);
    }
    if (u16ClrVal & HAL_DISP_IRQ_DMAMISC_ALLEMPTY_BIT)
    {
        DISP_STATISTIC_VAL(g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u32CmdFIFOEmptyCnt++);
        DISP_STATISTIC_VAL(g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u64TimeStamp[E_HAL_DISP_IRQ_DMA_CMDEMPTY] = u64Time);
    }
    HalDispUtilityW2BYTEMSKDirectAndNoshadow(DISP_SGWDMA_REG_WEVENT_CLR, u16ClrVal, mask_of_disp_sgwdma_reg_wevent_clr);
    HalDispUtilityW2BYTEMSKDirectAndNoshadow(DISP_WDMA_REG_SC_FRM_W_INT_CLR, mask_of_disp_wdma_reg_sc_frm_w_int_clr,
                                             mask_of_disp_wdma_reg_sc_frm_w_int_clr);
}
...
static __inline void _HalDispIrqGetDmaFlag(HalDispIrqType_e enType, u32 *pu32Flag, u32 u32DmaId)
{
    ...
    *pu32Flag = 0;
    ...
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_ACTIVE_ON)
    {
        if (u16Val & HAL_DISP_IRQ_DMA_VSYNC_MSK)
        {
            u16ClrVal |= (HAL_DISP_IRQ_DMA_VSYNC_BIT);
            if (HAL_DISP_IS_IRQ_EN(u16Msk, HAL_DISP_IRQ_DMA_VSYNC_MSK))
            {
    ...
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_REALDONE)
    {
        if (u16Val & HAL_DISP_IRQ_DMA_REALDONE_MSK)
        {
            *pu32Flag |= E_HAL_DISP_IRQ_TYPE_DMA_REALDONE;
            u16ClrVal |= (HAL_DISP_IRQ_DMA_REALDONE_BIT | HAL_DISP_IRQ_DMA_BLANKING_BIT);
            ...
            HalDispDmaGetFrmWCrc16(g_stDispIrqHist.stWorkingStatus.stDmaStatus[u32DmaId].u32CRC16);
            g_u8DvContinue = 0;
            g_stDispIrqHist.stWorkingStatus.stDmaStatus[u32DmaId].u8bDmaIdle = 1;
        }
        if (u16Val & HAL_DISP_IRQ_DMA_BLANKING_MSK)
        {
            ...
            u16ClrVal |= HAL_DISP_IRQ_DMA_BLANKING_BIT;
        }
    }
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_FIFOFULL)
    {
    ...
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_TRIG_FAIL)
    {
        if (u16Val & HAL_DISP_IRQ_DMA_TRIG_FAIL_BIT)
        {
            u16ClrVal |= HAL_DISP_IRQ_DMA_TRIG_FAIL_BIT;
            DISP_STATISTIC_VAL(g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u32TrigFailCnt++);
            DISP_STATISTIC_VAL(g_stDispIrqHist.stWorkingHist.stDmaHist[u32DmaId].u64TimeStamp[E_HAL_DISP_IRQ_DMA_TRIGFAIL] = u64Time);
            HalDispDmaSetFrmWTrigger(1, NULL);
        }
    }
    if (enType & E_HAL_DISP_IRQ_TYPE_DMA_MISC)
    {
        if (u16Val & HAL_DISP_IRQ_DMA_MISC_BIT)
        {
            u16ClrVal |= HAL_DISP_IRQ_DMA_MISC_BIT;
            _HalDispIrqDmaMiscHandler(u64Time, u32DmaId);
        }
    }
    // Clear, 1 Clear
...
static __inline void _HalDispIrqGetTimeZoneIsrIdx(u8 *pu8Id, u32 u32DevId)
{
    *pu8Id = (u32DevId == HAL_DISP_DEVICE_ID_1)   ? HAL_DISP_IRQ_ID_TIMEZONE_1
             : (u32DevId == HAL_DISP_DEVICE_ID_0) ? HAL_DISP_IRQ_ID_TIMEZONE_0
                                                  : HAL_DISP_IRQ_ID_MAX;
}
            ...
            if (_HalDispIrqHandleFpllCheckLocked(u32FpllId, pstDevStatus) == 0)
            {
                ...
                if((pstDevStatus->u16PhaseErr[u32FpllId]) == 0x7FFF || (pstDevStatus->u16PhaseErr[u32FpllId]) == 0x8000)
                {
                    ...
                    if(0==(g_stDispIrqHist.stWorkingStatus.stFpllStatus[u32FpllId].u32PhaseErrCnt % HAL_DISP_FPLL_CONTINUE_ERR_PHASE_CNT))
                    {
                        ...
                        DISP_DBG_STAT(DISP_DBG_LEVEL_IRQ_FPLL,
                                 "%s %d, Id: %d, PhaseErrCnt: %u  ,PhaseErrRstCnt: %u ,FpllRstCnt: %u FpllUstableCnt:%hu @%llu\n",
                                 __FUNCTION__, __LINE__, u32FpllId, g_stDispIrqHist.stWorkingStatus.stFpllStatus[u32FpllId].u32PhaseErrCnt,
...
{
    u16                            u16Val = 0, u16MskVal = 0, u16ClrVal;
    ...
    *pu32Flag = 0;
    ...
    if (enType & E_HAL_DISP_IRQ_TYPE_TIMEZONE_VDE_NEGATIVE)
    {
        ...
        {
            DISP_STATISTIC_VAL(u32VdeCnt = g_stDispIrqHist.stWorkingHist.stTimeZoneHist[u32DevId].u32VDeDoneCnt);
            if (u32VdeCnt % 1000 == 0)
            {
...
{
    g_stDispIrqHist.stWorkingStatus.stDmaStatus[0].u16Crc16Md = HAL_DISP_REG_CRC_MD_RIU;
