...
U32 eMMC_CSD_Config(void)
{
    ...
    g_eMMCDrv.u8_WP_GRP_SIZE = ((g_eMMCDrv.au8_CSD[11]&0xF0)>>3 | (g_eMMCDrv.au8_CSD[12]&0x01));
    g_eMMCDrv.u8_WP_GRP_ENABLE = (g_eMMCDrv.au8_CSD[12]&0x02)>>1;
...
U32 eMMC_ExtCSD_Config(void)
{
    ...
    if(0 == g_eMMCDrv.u32_SEC_COUNT)
         g_eMMCDrv.u32_SEC_COUNT = ((gau8_eMMC_SectorBuf[215]<<24)|
                                   ...
                                   (gau8_eMMC_SectorBuf[212])); //cancel -8: Toshiba CMD18 access the last block report out of range error
    ...
    g_eMMCDrv.u8_ECSD175_ERASE_GROUP_DEF = gau8_eMMC_SectorBuf[175];

    //for WP
    g_eMMCDrv.u8_ECSD174_BootWPStatus = gau8_eMMC_SectorBuf[174];
    g_eMMCDrv.u8_ECSD173_BootWP = gau8_eMMC_SectorBuf[173];
    g_eMMCDrv.u8_ECSD171_UserWP = gau8_eMMC_SectorBuf[171];
    if(g_eMMCDrv.u8_IfSectorMode)
        g_eMMCDrv.u32_WP_group_size = (g_eMMCDrv.u8_ECSD221_HCWpGRPSize * g_eMMCDrv.u8_ECSD224_HCEraseGRPSize * 0x400);
    else
        g_eMMCDrv.u32_WP_group_size = ((g_eMMCDrv.u8_ERASE_GRP_SIZE+1)*(g_eMMCDrv.u8_WP_GRP_SIZE+1));
...
U32 eMMC_CMD28_CMD29(U32 u32_eMMCDataAddress1, U8 u8_CmdIdx)
{
    U32 u32_err, u32_arg;
    U16 u16_ctrl, u16_reg;
    U8  u8_retry_r1=0, u8_retry_fcie=0, u8_retry_cmd=0;

    u32_arg = u32_eMMCDataAddress1 << (g_eMMCDrv.u8_IfSectorMode ? 0 : eMMC_SECTOR_512BYTE_BITS);
    u16_ctrl = BIT_SD_CMD_EN | BIT_SD_RSP_EN;
    g_eMMCDrv.u32_DrvFlag |= DRV_FLAG_RSP_WAIT_D0H;
    ...
    eMMC_FCIE_ClearEvents();
    u32_err = eMMC_FCIE_SendCmd(
        g_eMMCDrv.u16_Reg10_Mode, u16_ctrl, u32_arg, u8_CmdIdx, eMMC_R1b_BYTE_CNT);
    if(eMMC_ST_SUCCESS != u32_err)
    {
        if(u8_retry_cmd < eMMC_FCIE_CMD_RSP_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
        {
            u8_retry_cmd++;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD%d  retry:%u, %Xh \n", u8_CmdIdx, u8_retry_cmd, u32_err);
            eMMC_FCIE_ErrHandler_Retry();
            goto LABEL_SEND_CMD;
        }
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD%d  retry:%u, %Xh \n", u8_CmdIdx, u8_retry_cmd, u32_err);
        eMMC_FCIE_ErrHandler_Stop();
    }
    else
    {   // check status
        ...
        if(u16_reg & (BIT_SD_RSP_TIMEOUT|BIT_SD_RSP_CRC_ERR))
        {
            if(u8_retry_fcie < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
            {
                u8_retry_fcie++;
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                    "eMMC WARN: CMD%d  SD_STS: %04Xh, Retry: %u\n", u8_CmdIdx, u16_reg, u8_retry_fcie);
            ...
            u32_err = eMMC_ST_ERR_CMD6;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                "eMMC Err: CMD%d  SD_STS: %04Xh, Retry: %u\n", u8_CmdIdx, u16_reg, u8_retry_fcie);
            eMMC_FCIE_ErrHandler_Stop();
        }
        else
        {
            ...
            if(eMMC_ST_SUCCESS != u32_err)
            {
                if(u8_retry_r1 < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE()){
                    u8_retry_r1++;
                    eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                        "eMMC WARN: CMD%d check R1 error: %Xh, retry: %u\n", u8_CmdIdx, u32_err, u8_retry_r1);
                    eMMC_FCIE_ErrHandler_Retry();
                    goto LABEL_SEND_CMD;
                }
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                    "eMMC Err: CMD%d  check R1 error: %Xh, retry: %u\n", u8_CmdIdx, u32_err, u8_retry_r1);
                eMMC_FCIE_ErrHandler_Stop();
            }
        }
    }
...
U32 eMMC_CMD30_CMD31_MIU(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf, U8 u8_CmdIdx)
{
    U32 u32_err, u32_arg;
    U16 u16_mode, u16_ctrl, u16_reg;
    U8  u8_retry_fcie=0, u8_retry_r1=0, u8_retry_cmd=0;
    U32 u32_dma_addr, u32_blksize;
    ...
    eMMC_FCIE_ClearEvents();
    REG_FCIE_W(FCIE_JOB_BL_CNT, 1);
    u32_dma_addr = eMMC_translate_DMA_address_Ex((U32)pu8_DataBuf, u32_blksize, READ_FROM_eMMC);

    REG_FCIE_W(FCIE_MIU_DMA_ADDR_15_0, u32_dma_addr & 0xFFFF);
    REG_FCIE_W(FCIE_MIU_DMA_ADDR_31_16, (u32_dma_addr >> 16) & 0xFFFF);

    REG_FCIE_W(FCIE_MIU_DMA_LEN_15_0, u32_blksize & 0xFFFF);
    REG_FCIE_W(FCIE_MIU_DMA_LEN_31_16, u32_blksize >> 16);
    REG_FCIE_W(FCIE_BLK_SIZE,  u32_blksize);

    u32_err = eMMC_FCIE_SendCmd(
        u16_mode, u16_ctrl, u32_arg, u8_CmdIdx, eMMC_R1_BYTE_CNT);
    if(eMMC_ST_SUCCESS != u32_err)
    {
        if( !g_eMMCDrv.u8_disable_retry )
        {
            ...
            {
                u8_retry_cmd++;
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD%d retry:%u, %Xh, Arg: %Xh \n", u8_CmdIdx, u8_retry_cmd, u32_err, u32_arg);
                eMMC_FCIE_ErrHandler_Retry();
                goto LABEL_SEND_CMD;
            }

            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD%d retry:%u, %Xh, Arg: %Xh \n", u8_CmdIdx, u8_retry_cmd, u32_err, u32_arg);
            eMMC_FCIE_ErrHandler_Stop();
            goto LABEL_END;
        }
        else
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD%d %Xh, Arg: %Xh \r\n", u8_CmdIdx, u32_err, u32_arg);
            eMMC_FCIE_ErrHandler_Stop();
            goto LABEL_END;
        }
    }
    ...
    if(eMMC_ST_SUCCESS != u32_err || (u16_reg & BIT_SD_FCIE_ERR_FLAGS))
    {
        if( !g_eMMCDrv.u8_disable_retry )
        {
            if(u8_retry_fcie < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
            {
                u8_retry_fcie++;
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                    "eMMC WARN: CMD%d SD_STS: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n", u8_CmdIdx, u16_reg, u32_err, u8_retry_fcie, u32_arg);
                eMMC_FCIE_ErrHandler_Retry();
                goto LABEL_SEND_CMD;
            }
            ...
            u32_err = eMMC_ST_ERR_CMD17_MIU;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                "eMMC Err: CMD%d SD_STS: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n", u8_CmdIdx, u16_reg, u32_err, u8_retry_fcie, u32_arg);
            eMMC_FCIE_ErrHandler_Stop();
            goto LABEL_END;
        }
        else
        {
            u32_err = eMMC_ST_ERR_CMD17_MIU;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                "eMMC Err: CMD%d SD_STS: %04Xh, Err: %Xh, Arg: %Xh\r\n", u8_CmdIdx, u16_reg, u32_err, u32_arg);
            REG_FCIE_CLRBIT(FCIE_RSP_SHIFT_CNT, BIT_RSP_SHIFT_SEL|BIT_RSP_SHIFT_TUNE_MASK);
            eMMC_FCIE_ErrHandler_Stop();
            goto LABEL_END;
        }
    ...
    if(eMMC_ST_SUCCESS != u32_err)
    {
        if( !g_eMMCDrv.u8_disable_retry )
        {
            if(u8_retry_r1 < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
            {
                u8_retry_r1++;
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                    "eMMC WARN: CMD%d check R1 error: %Xh, Retry: %u, Arg: %Xh\n", u8_CmdIdx, u32_err, u8_retry_r1, u32_arg);
                eMMC_FCIE_ErrHandler_Retry();
                goto LABEL_SEND_CMD;
            }
            else
            {   u32_err = eMMC_ST_ERR_CMD17_MIU;
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                    "eMMC Err: CMD%d check R1 error: %Xh, Retry: %u, Arg: %Xh\n", u8_CmdIdx, u32_err, u8_retry_r1, u32_arg);
                eMMC_FCIE_ErrHandler_Stop();
                goto LABEL_END;
            }
        }
        else
        {
            u32_err = eMMC_ST_ERR_CMD17_MIU;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                "eMMC Err: CMD%d check R1 error: %Xh, Arg: %Xh\r\n", u8_CmdIdx, u32_err, u32_arg);
            eMMC_FCIE_ErrHandler_Stop();
            goto LABEL_END;
        }
    }
