...
#endif
#if defined(CONFIG_SSTAR_INI_PARSER) && defined(CONFIG_FS_LITTLEFS)
#include "vsprintf.h"
#include "littlefs.h"
#include "iniparser.h"
...
static INI_FILE *lfs_ini_open(const char *path, const char *opt);
static int lfs_ini_close(INI_FILE *pfile_desc);
static char *lfs_ini_fgets(char *str_buf, int size, INI_FILE *file_desc);
...
static int _MiscPartiton(u32 u32LogoId, DispOutAspectRatio *pstAspectRatio, LogoRotation_e eRot)
{
...
#endif
    if (littlefs_mount(strENVName, NULL) < 0)
        return -1;
    ini_opt.ini_open = lfs_ini_open;
    ini_opt.ini_close = lfs_ini_close;
    ini_opt.ini_fgets = lfs_ini_fgets;
    ...
    void *fd = littlefs_open(pFileName, O_RDONLY, 0);
    if (fd == NULL)
    {
    ...
    u32FileSize = littlefs_lseek(fd, 0, SEEK_END);
    ...
    u32FileSize = littlefs_read(fd, inBuf, u32FileSize);
...
static INI_FILE *lfs_ini_open(const char *path, const char *opt)
{
    ...
    return (INI_FILE *)littlefs_open((char *)path, flags, 0);
}
static char *lfs_ini_fgets(char *str_buf, int size, INI_FILE *file_desc)
{
    char c;
    ...
    while(littlefs_read((void *)file_desc, &c, 1))
    {
        str_buf[read_cnt] = c;
        read_cnt++;
        if (read_cnt > size)
...
static int lfs_ini_close(INI_FILE *pfile_desc)
{
    return littlefs_close((void *)pfile_desc);
}
