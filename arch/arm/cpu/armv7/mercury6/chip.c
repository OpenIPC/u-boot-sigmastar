...
void Chip_Set_RebootType(u16 arg)
{
    if (arg==MS_REBOOT_BY_SW_RST)
    {
...
int Chip_Get_RebootType(void)
{
    U16 RegVal=0, rebootType=0;

    RegVal = INREG16(REG_ADDR_BASE_WDT+REG_ID_02);
    if(RegVal&0x01)
        rebootType=MS_REBOOT_BY_WDT_RST;
    else
    {
        RegVal = INREG16(REG_ADDR_BASE_PMPOR+REG_ID_01);
        if(RegVal&0x01)
            rebootType=MS_REBOOT_BY_SW_RST;
        else
            rebootType=MS_REBOOT_BY_HW_RST;
    }
    return rebootType;
}
