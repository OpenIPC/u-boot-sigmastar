...
#define CROP_V_START_ALIGN(x) ((x >> 5) << 5)
#define CROP_V_END_ALIGN(x)   (((x) & 0x1F) ? ((((x) >> 5) + 1) << 5) : (x))

#define CROP_H_START_ALIGN(x) ((x >> 5) << 5)
#define CROP_H_END_ALIGN(x)   (((x) & 0x01F) ? ((((x) >> 5) + 1) << 5) : (x))

#define TILE_BLOCK_HEIGHT_ALIGN(x) (((x) & 0x1F) ? ((((x) >> 5) + 1) << 5 ) : (x)) // 32

#define TILE_BLOCK_WIDHT_ALIGN(x) (((x) & 0x1F) ? ((((x) >> 5) + 1) << 5 ) : (x)) // 32

#define ROT_WDMA_STRIDE_ALIGN(x) (((x) & 0x1FF) ? ((((x) >> 9) + 1) << 9) : (x))
#define ROT_Y_IMI_LINE_COUNT     (32)
#define ROT_C_IMI_LINE_COUNT     (32)

#define MOP_V_SCALE_RATIO(in, out) (((in) * 4096) / (out))
#define MOP_H_SCALE_RATIO(in, out) ((((in) - 2) * 4096) / ((out) - 1))
//-------------------------------------------------------------------------------------------------
//  structure
//-------------------------------------------------------------------------------------------------
typedef struct
...
static void _HalDispMopIfSetMopRotImiAddrManage(HalDispMopId_e eMopId, u8 bMopRotId, u32 u32Ysize,
                                                u32 u32Csize) // Id=0: Rot0; Id=1: Rot1
{
    u32 u32TmpAddr = 0;

    DISP_DBG(DISP_DBG_LEVEL_HAL_MOP, "%s %d, rot_id=%d, Ysize=0x%x, Csize=0x%x\n", __FUNCTION__, __LINE__, bMopRotId,
             u32Ysize, u32Csize);
    // imi property: MOP0_Rot0_Y -> MOP0_Rot0_C -> MOP0_Rot1_Y -> MOP0_Rot1_C -> MOP1_Rot0_Y -> MOP1_Rot0_C ->
    // MOP1_Rot1_Y -> MOP1_Rot1_C
    if (bMopRotId == 0) // Rot0
    {
        u32TmpAddr                = g_u32Rot1ImiYaddr[eMopId];
        g_u32Rot0ImiCaddr[eMopId] = g_u32Rot0ImiYaddr[eMopId] + u32Ysize;
        g_u32Rot1ImiYaddr[eMopId] = g_u32Rot0ImiCaddr[eMopId] + u32Csize;
        g_u32Rot1ImiCaddr[eMopId] = g_u32Rot1ImiCaddr[eMopId] + g_u32Rot1ImiYaddr[eMopId] - u32TmpAddr;
        if (eMopId == E_HAL_DISP_MOPID_00)
        {
            g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] + g_u32Rot1ImiYaddr[eMopId] - u32TmpAddr;
            g_u32Rot0ImiCaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot0ImiCaddr[E_HAL_DISP_MOPID_01] + g_u32Rot1ImiYaddr[eMopId] - u32TmpAddr;
            g_u32Rot1ImiYaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot1ImiYaddr[E_HAL_DISP_MOPID_01] + g_u32Rot1ImiYaddr[eMopId] - u32TmpAddr;
            g_u32Rot1ImiCaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot1ImiCaddr[E_HAL_DISP_MOPID_01] + g_u32Rot1ImiYaddr[eMopId] - u32TmpAddr;
        }
    }
    else // Rot1
    {
        g_u32Rot1ImiCaddr[eMopId] = g_u32Rot1ImiYaddr[eMopId] + u32Ysize;
        if (eMopId == E_HAL_DISP_MOPID_00)
        {
            u32TmpAddr                             = g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01];
            g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] = g_u32Rot1ImiCaddr[eMopId] + u32Csize;
            g_u32Rot0ImiCaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot0ImiCaddr[E_HAL_DISP_MOPID_01] + g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] - u32TmpAddr;
            g_u32Rot1ImiYaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot1ImiYaddr[E_HAL_DISP_MOPID_01] + g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] - u32TmpAddr;
            g_u32Rot1ImiCaddr[E_HAL_DISP_MOPID_01] =
                g_u32Rot1ImiCaddr[E_HAL_DISP_MOPID_01] + g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] - u32TmpAddr;
        }
    }
}
...
static void _HalDispMopIfPollingDeEnd(DrvDispCtxConfig_t *pstDispCtxCfg, HalDispMopId_e eMopId)
{
    HalDispUtilityCmdqContext_t *pstCmdqCtx = NULL;

    HalDispGetCmdqByCtx((void *)pstDispCtxCfg, (void *)&pstCmdqCtx);

    HalDispUtilityWritePollCmd(HAL_DISP_UTILITY_DISP_TO_CMDQDEV_RAW(pstCmdqCtx->s32UtilityId),
                           HAL_DISP_CMDQIRQ_DEV_OP2VDE_OFF_BIT, HAL_DISP_CMDQIRQ_DEV_OP2VDE_OFF_MSK, 2000000000, 1,
                           pstCmdqCtx);
}
...
// Global Functions
//-------------------------------------------------------------------------------
HalDispQueryRet_e HalDispMopIfGetInfoVidLayerInit(void *pCtx, void *pCfg)
{
    ...
    {
        memset(&g_pMopgLbOdTbl[eMopId][0], 0x00, sizeof(HalDispMopgLineBufOrder_t) * E_HAL_DISP_MOPG_ID_NUM);
        g_bMopgInitNum[eMopId] = 0;
    }
...
HalDispQueryRet_e HalDispMopIfGetInfoInputPortEnable(void *pCtx, void *pCfg)
{
    ...
    pstInputPortContain = pstDispCtxCfg->pstCtxContain->pstInputPortContain[pstDispCtxCfg->u32ContainIdx];
    pstVidLayerContain  = (DrvDispCtxVideoLayerContain_t *)pstInputPortContain->pstVidLayerContain;
    u32VidLayerId = pstVidLayerContain->u32VidLayerId;
    pbEnable            = (u8 *)pCfg;
...
void HalDispMopIfSetInputPortAttr(void *pCtx, void *pCfg)
{
        ...
        && (pstInputPortContain->u32PortId < E_HAL_DISP_MOPG_ID_NUM)) // mopg
    {
        ...
        {
            u16Hratio = MOP_H_SCALE_RATIO(pstHalInputPortCfg->u16SrcWidth, pstHalInputPortCfg->stDispWin.u16Width);
        ...
        {
            u16Vratio = MOP_V_SCALE_RATIO(pstHalInputPortCfg->u16SrcHeight, pstHalInputPortCfg->stDispWin.u16Height);
             ...
             && (pstInputPortContain->u32PortId == E_HAL_DISP_MOPS_ID_00)) // mops
    {
        ...
        if (pstHalInputPortCfg->stDispWin.u16Height > pstHalInputPortCfg->u16SrcHeight)
        {
            u16Vratio = MOP_V_SCALE_RATIO(pstHalInputPortCfg->u16SrcHeight,
...
void HalDispMopIfSetInputPortFlip(void *pCtx, void *pCfg)
{
    ...
    if (pstInputPortContain->stRot.enRotate == E_HAL_DISP_ROTATE_90 ||
        pstInputPortContain->stRot.enRotate == E_HAL_DISP_ROTATE_270)
    {
        ...
        _HalDispMopIfPollingDeEnd(pstDispCtxCfg, eMopId);
    }
    else
    {
        ...
        && (pstInputPortContain->u32PortId < E_HAL_DISP_MOPG_ID_NUM)) // mopg
    {
            ...
            && pstVidLayerContain->u32VidLayerId != E_HAL_DISP_LAYER_MOPG_VIDEO2_ID)
        {
            ...
            // rot_r source_width=source_width align 32
            u16tempRotInW = TILE_BLOCK_WIDHT_ALIGN(pstInputPortContain->stCrop.u16X + pstInputPortContain->stAttr.u16SrcWidth);
            ...
            // new way
            u16tempRotInH = CROP_V_END_ALIGN(pstInputPortContain->stCrop.u16Y + pstInputPortContain->stAttr.u16SrcHeight)
                            - CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16Y);
            ...
            HalDispMopgSetYAddr(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId,
                                g_u32Rot0ImiYaddr[eMopId]);
            HalDispMopgSetCAddr(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId,
                                g_u32Rot0ImiCaddr[eMopId]);
            // pitch is set in rotate/crop config
            // crop offset 16 change to dummy pixel in imi
            u8AddrOffset = (u8)(pstInputPortContain->stCrop.u16X & 0x1F);
            ...
            if (eMopId == E_HAL_DISP_MOPID_00)
            {
                ...
                if (eMopId == E_HAL_DISP_MOPID_00)
                {
                    ...
                    //if device 1 open rotate
                    if (g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01] != g_u32Rot0ImiCaddr[E_HAL_DISP_MOPID_01])
                    {
                        //set MOPG read imi address
                        HalDispMopgSetYAddr(pCtx, E_HAL_DISP_MOPID_01, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId,
                                            g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01]);
                        HalDispMopgSetCAddr(pCtx, E_HAL_DISP_MOPID_01, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId,
                                            g_u32Rot0ImiCaddr[E_HAL_DISP_MOPID_01]);
                    }

                    if (g_u32Rot1ImiYaddr[E_HAL_DISP_MOPID_01] != g_u32Rot1ImiCaddr[E_HAL_DISP_MOPID_01])
                    {
                        //set MOPG read imi address
                 ...
                 && pstVidLayerContain->u32VidLayerId != E_HAL_DISP_LAYER_MOPG_VIDEO2_ID)
        {
            ...
            u8AddrOffset = u16tempRotInW - u8AddrOffset - pstInputPortContain->stAttr.u16SrcWidth
                           - CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16X);
            HalDispMopRot0SetPixDummy(pCtx, eMopId, u8AddrOffset);
        ...
        else
        {
            ...
            u8AddrOffset = (u8)(pstInputPortContain->stCrop.u16X & 0x0F);
             ...
             && (pstInputPortContain->u32PortId == E_HAL_DISP_MOPS_ID_00)) // mops
    {
            ...
            && pstVidLayerContain->u32VidLayerId != E_HAL_DISP_LAYER_MOPS_VIDEO2_ID)
        {
            ...
            if (eMopId == E_HAL_DISP_MOPID_00)
            {
                //set rot_r write imi address
                HalDispMopRot0SetWriteYAddr(pCtx, E_HAL_DISP_MOPID_01, g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01]);
                HalDispMopRot0SetWriteCAddr(pCtx, E_HAL_DISP_MOPID_01, g_u32Rot0ImiYaddr[E_HAL_DISP_MOPID_01]);
                //set rot_r write imi address
                HalDispMopRot1SetWriteYAddr(pCtx, E_HAL_DISP_MOPID_01, g_u32Rot1ImiYaddr[E_HAL_DISP_MOPID_01]);
                HalDispMopRot1SetWriteCAddr(pCtx, E_HAL_DISP_MOPID_01, g_u32Rot1ImiCaddr[E_HAL_DISP_MOPID_01]);
                //if device 1 open rotate
...
HalDispQueryRet_e HalDispMopIfGetInfoInputPortRotate(void *pCtx, void *pCfg)
{
        ...
        && (pstInputPortContain->u32PortId < E_HAL_DISP_MOPG_ID_NUM)) // mopg
    {
        ...
        else
        {
            if (pstHalInputPortCfg->enRotate == E_HAL_DISP_ROTATE_NONE)
            {
                ...
                HalDispMopgSetDmaYFifoThrd(pCtx, eMopId, g_au8MopYDmaThreshold[eMopId], 0); // h04 [7:0]ymd_thd
                HalDispMopgSetDmaCFifoThrd(pCtx, eMopId, g_au8MopCDmaThreshold[eMopId], 0); // h05 [7:0]cdma_thd; default=0xf8, use 0xd0
             ...
             && (pstInputPortContain->u32PortId == E_HAL_DISP_MOPS_ID_00)) // mops
    {
        ...
        {
            HalDispMopsSetDmaYFifoThrd(pCtx, eMopId, g_au8MopYDmaThreshold[eMopId], 0); // h04 [7:0]ymd_thd
            HalDispMopsSetDmaCFifoThrd(pCtx, eMopId, g_au8MopCDmaThreshold[eMopId], 0); // h05 [7:0]cdma_thd; default=0xf8, use 0xd0
...
void HalDispMopIfSetInputPortRotate(void *pCtx, void *pCfg)
{
        ...
        && (pstInputPortContain->u32PortId < E_HAL_DISP_MOPG_ID_NUM)) // mopg
    {
        if (pstHalInputPortCfg->enRotate == E_HAL_DISP_ROTATE_NONE)
        {
            ...
            // reset rotate crop case
            HalDispMopRot0SetRotateCropXEn(pCtx, eMopId, 0);
            HalDispMopRot0SetRotateCropYEn(pCtx, eMopId, 0);
        }
        else if (pstHalInputPortCfg->enRotate == E_HAL_DISP_ROTATE_90)
        {
            _HalDispMopIfPollingDeEnd(pstDispCtxCfg, eMopId);
            ...
            u16tempRotInH = TILE_BLOCK_HEIGHT_ALIGN(pstInputPortContain->stAttr.u16SrcHeight);
            HalDispMopRot0SourceHeight(pCtx, eMopId, u16tempRotInH);

            // pitch is the same as source width, but 512 align
            u16tempRotInH = ROT_WDMA_STRIDE_ALIGN(pstInputPortContain->stAttr.u16SrcHeight);
            HalDispMopgSetPitch(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId, u16tempRotInH);

            u16RotDummyPix = pstInputPortContain->stAttr.u16SrcHeight & 0x0F;
            ...
            {
                u16Hratio = MOP_H_SCALE_RATIO(pstInputPortContain->stAttr.u16SrcHeight,
            ...
            {
                u16Vratio = MOP_V_SCALE_RATIO(pstInputPortContain->stAttr.u16SrcWidth,
             ...
             && (pstInputPortContain->u32PortId == E_HAL_DISP_MOPS_ID_00)) // mops
    {
        if (pstHalInputPortCfg->enRotate == E_HAL_DISP_ROTATE_NONE)
        {
            ...
            // reset rotate crop case
            HalDispMopRot1SetRotateCropXEn(pCtx, eMopId, 0);
            HalDispMopRot1SetRotateCropYEn(pCtx, eMopId, 0);
        ...
        else if (pstHalInputPortCfg->enRotate == E_HAL_DISP_ROTATE_90)
        {
            ...
            u16RotDummyPix = (pstInputPortContain->stAttr.u16SrcHeight & 0xF);
...
HalDispQueryRet_e HalDispMopIfGetInfoInputPortCrop(void *pCtx, void *pCfg)
{
    ...
    DISP_DBG(DISP_DBG_LEVEL_HAL_MOP, "%s %d, Crop align (%x %x %x %x)\n", __FUNCTION__, __LINE__,
              CROP_H_START_ALIGN(pstHalInputPortCfg->u16X),
              CROP_V_START_ALIGN(pstHalInputPortCfg->u16Y),
              CROP_H_END_ALIGN(pstHalInputPortCfg->u16X+ pstHalInputPortCfg->u16Width),
              CROP_V_END_ALIGN(pstHalInputPortCfg->u16Y + pstHalInputPortCfg->u16Height));
...
void HalDispMopIfSetInputPortCrop(void *pCtx, void *pCfg)
{
        ...
        && (pstInputPortContain->u32PortId < E_HAL_DISP_MOPG_ID_NUM)) // mopg
    {
            ...
            || (pstInputPortContain->stRot.enRotate == E_HAL_DISP_ROTATE_270)) // rotate case
        {
            ...
            u16tmpHstForCR = CROP_H_START_ALIGN(pstInputPortContain->stCrop.u16X);
            u16tmpVstForCR = CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16Y);
            u16tmpHendForCR =
                CROP_H_END_ALIGN(pstInputPortContain->stCrop.u16X + pstInputPortContain->stAttr.u16SrcWidth);
            u16tmpVendForCR =
                CROP_V_END_ALIGN(pstInputPortContain->stCrop.u16Y + pstInputPortContain->stAttr.u16SrcHeight);
            ...
            {
                u16Hratio = MOP_H_SCALE_RATIO(pstHalInputPortCfg->u16Height,
                                              pstInputPortContain->stAttr.stDispWin.u16Width);
                HalDispMopgSetHScaleFac(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId, u16Hratio);
            }
            else
            ...
            {
                u16Vratio = MOP_V_SCALE_RATIO(pstHalInputPortCfg->u16Width,
                                              pstInputPortContain->stAttr.stDispWin.u16Height);
                HalDispMopgSetVScaleFac(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId, u16Vratio);
            }
            else
            ...
            // Rot_r input_width=pitch, height=source_height align 16
            u16tempRotInH = TILE_BLOCK_HEIGHT_ALIGN(pstInputPortContain->stCrop.u16Y + pstInputPortContain->stAttr.u16SrcHeight);
            HalDispMopRot0SourceHeight(pCtx, eMopId, u16tempRotInH);

            // pitch is the same as source width, but 512 align
            // refined
            u16tempPitch = u16tempRotInH - CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16Y);
            ...
            if ((pstInputPortContain->stAttr.u16SrcWidth == 0) || (pstInputPortContain->stAttr.u16SrcHeight == 0))
            {
                HalDispMopRot0SetRotateCropXEn(pCtx, eMopId, 0);
                HalDispMopRot0SetRotateCropYEn(pCtx, eMopId, 0);
            }
            ...
            {
                // refined
                u16RotDummyPix = pstInputPortContain->stCrop.u16Y - CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16Y);
                HalDispMopgSetAddr16Offset(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId,
                                           u16RotDummyPix);
            }
            else
        ...
        else // not rotate case
        {
            ...
            {
                u16Hratio = MOP_H_SCALE_RATIO(pstHalInputPortCfg->u16Width,
                                              pstInputPortContain->stAttr.stDispWin.u16Width);
                HalDispMopgSetHScaleFac(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId, u16Hratio);
            }
            else
            ...
            {
                u16Vratio = MOP_V_SCALE_RATIO(pstHalInputPortCfg->u16Height,
                                              pstInputPortContain->stAttr.stDispWin.u16Height);
                HalDispMopgSetVScaleFac(pCtx, eMopId, (HalDispMopgGwinId_e)pstInputPortContain->u32PortId, u16Vratio);
            }
            else
             ...
             && (pstInputPortContain->u32PortId == E_HAL_DISP_MOPS_ID_00)) // mops
    {
        ...
        {
            // count mop source W/H for crop+ rotate case
            u16tmpHstForCR = CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16X);
            u16tmpVstForCR = CROP_V_START_ALIGN(pstInputPortContain->stCrop.u16Y);
            u16tmpHendForCR =
                CROP_V_END_ALIGN(pstInputPortContain->stCrop.u16X + pstInputPortContain->stAttr.u16SrcWidth);
            u16tmpVendForCR =
                CROP_V_END_ALIGN(pstInputPortContain->stCrop.u16Y + pstInputPortContain->stAttr.u16SrcHeight);
            u16tmpWidthForCR  = u16tmpHendForCR - u16tmpHstForCR;
            u16tmpHeightForCR = u16tmpVendForCR - u16tmpVstForCR;
            ...
            // Rot_r input_width=pitch, height=source_height align 16
            u16tempRotInH = TILE_BLOCK_HEIGHT_ALIGN(pstInputPortContain->stCrop.u16Y +
                                                    pstInputPortContain->stAttr.u16SrcHeight);
            HalDispMopRot1SourceHeight(pCtx, eMopId, u16tempRotInH);
...
void HalDispMopIfSetYDmaThreshold(void *pCtx, HalDispMopId_e enMopId, u8 u8Threshold)
{
    HalDispMopSetYDmaThreshold(enMopId, u8Threshold);
    HalDispMopgSetDmaYFifoThrd(pCtx, enMopId, u8Threshold, 1);
    HalDispMopsSetDmaYFifoThrd(pCtx, enMopId, u8Threshold, 1);
}

void HalDispMopIfSetCDmaThreshold(void *pCtx, HalDispMopId_e enMopId, u8 u8Threshold)
{
    HalDispMopSetCDmaThreshold(enMopId, u8Threshold);
    HalDispMopgSetDmaCFifoThrd(pCtx, enMopId, u8Threshold, 1);
    HalDispMopsSetDmaCFifoThrd(pCtx, enMopId, u8Threshold, 1);
}
