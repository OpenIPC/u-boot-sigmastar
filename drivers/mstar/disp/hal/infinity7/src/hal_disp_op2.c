...
void HalDispSetVgaDtopDumpByDiv(u32 u32Mulit, u32 u32Div, void *pCtx)
{
    ...
    HalDispGetCmdqByCtx(pCtx, &pCmdqCtx);
    HalDispUtilityW2BYTEMSK(DAC_ATOP_REG_SDIOPLL_LOOP_DIV, (u32Mulit * HAL_DISP_VGA_DCLKTOHWDCLKMULTIPLE / 2),
                            mask_of_dac_atop_reg_sdiopll_loop_div, pCmdqCtx);
    HalDispUtilityW2BYTEMSK(DAC_ATOP_REG_SDIOPLL_PDIV, HAL_DISP_GET_VGAPLLPOSTDIVIDX(u32Div),
                            mask_of_dac_atop_reg_sdiopll_pdiv, pCmdqCtx);
}
void HalDispSetHdmitxAtopDumpByDiv(u32 u32Mulit, u32 u32Div, void *pCtx)
{
    ...
    HalDispUtilityW2BYTEMSK(REG_HDMI_TX_ATOP_16_L, (u8DrivingUp ? 0x7f0 : 0xf0),
                            0xffff, pCmdqCtx);
    HalDispUtilityW2BYTEMSK(REG_HDMI_TX_ATOP_22_L, (u8DrivingUp ? 0x2b2b : 0x2828),
                            0xffff, pCmdqCtx);
    HalDispUtilityW2BYTEMSK(REG_HDMI_TX_ATOP_23_L, (u8DrivingUp ? 0x142b : 0x2828),
                            0xffff, pCmdqCtx);
...
void HalDispSetDacPllPowerDown(u16 u16Val, void *pCtx)
{
    ...
    HalDispGetCmdqByCtx(pCtx, &pCmdqCtx);
    HalDispUtilityW2BYTEMSKByType(DAC_ATOP_REG_SDIOPLL_PD, u16Val, mask_of_dac_atop_reg_sdiopll_pd, pCmdqCtx, E_DISP_UTILITY_REG_ACCESS_CMDQ);
    if (pCmdqCtx)
    {
...
{
    u32 u32Reg = u32DevId == HAL_DISP_DEVICE_ID_1   ? REG_DISP_MOPG_BK10_00_L
                 : u32DevId == HAL_DISP_DEVICE_ID_2 ? REG_DISP_MOPG_BK20_00_L
