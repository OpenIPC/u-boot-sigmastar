...
int do_sdstar     (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if(UpgradeImage == NULL)
    {
        ...
        //run_command("saveenv",0); //exeute this cmd may cause an error in the SPI flash
    }
    memset(tmp,0,sizeof(tmp));
    snprintf(tmp, sizeof(tmp) - 1,"fatload mmc %s %X %s 0x%x 0x0", devPart, (U32)buffer, UpgradeImage, USBUPGRDE_SCRIPT_BUF_SIZE);
    ...
    memset(tmp,0,sizeof(tmp));
    sprintf(tmp, "setenv MMC_Dev %s", devPart);
    run_command(tmp, 0);
...
#endif
#ifdef CONFIG_SSTAR_REG_CHIPID
    int do_estar_count=0;
#endif
...
//        U32 u32Percentage = 0; U32 u32Pos = 9; U32 u32TextColor = COLOR_GREEN;
...
#ifdef CONFIG_MS_NAND_ONEBIN || CONFIG_MS_NOR_ONEBIN
        char *tftp_ipl_line = "tftp 0x21000000 boot.bin";
#else
        char *tftp_ipl_line = "tftp 0x21000000 IPL.bin";
#endif
        char *reset_line = "reset";
        int matchret=0;
        ...
        if(do_estar_count==1)
        {
            // verify Reg_chipid
            printf ("\n>> %s \n",tftp_ipl_line);
            run_command(tftp_ipl_line, 0);
            matchret=match_chipid();
            if (matchret==-1)
            {
                printf("error!!reset soon，replace the burning image and try again!");
                mdelay(5000);
                run_command(reset_line, 0);
            }
        }
#endif
        while ((next_line = get_script_next_line(&script_buf)) != NULL)
...
static int do_emmc_get_cust(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    u32CustAddress = u32BootPartAddress + u32IplSize + RSA_SIGNATURE_LEN;
...
int do_emmc_wp(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    u32 u32_address, u32_size;
    u8  u8_mode;

    if((argc != 3) && (argc != 4))
        return CMD_RET_USAGE;

    u32_address = (unsigned int)simple_strtoul(argv[2], NULL, 16);
    u32_size = (unsigned int)simple_strtoul(argv[3], NULL, 16);

    if(strncmp(argv[1], "-s", 8) == 0)
        eMMC_USER_Set_WriteProtect(u32_address, u32_size);
    else if(strncmp(argv[1], "-c", 8) == 0)
        eMMC_USER_Clr_WriteProtect(u32_address, u32_size);
    else if(strncmp(argv[1], "-q", 8) == 0)
    {
        u8_mode = eMMC_USER_Get_WriteProtect(u32_address);
        printf("eMMC USER part address(0x%08x)'s write protection is %d\n", u32_address, u8_mode);
    }
    else if(strncmp(argv[1], "-qt", 8) == 0)
    {
        u8_mode = eMMC_USER_Get_WriteProtect_Type(u32_address);
...
U_BOOT_CMD(
    emmc_wp,  CONFIG_SYS_MAXARGS,    0,    do_emmc_wp,
    "eMMC write protection related",
    ...
    "    qt : Ask the address of write group's write protection type. (0:not protected, 1:temporary, 2:power-on, 3:permanent)\n"
...
int do_aesdma(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    {
        config.eKeyType = E_AESDMA_KEY_OTP_EFUSE_KEY2;
    }
    else if(strncmp(argv[4], "OTP", 3) == 0)
    {
        config.eKeyType = E_AESDMA_KEY_OTP_EFUSE_KEY1;
    }
    else
...
int pwmtest(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    polarity = simple_strtoul(argv[4], NULL, 0);
    pad = simple_strtoul(argv[5], NULL, 0);

    if (id < 0 || id >11 || duty < 0 || period_ns < 0)
    {
        printf("pwm error\n");
        return -1;
    }
 ...
 U_BOOT_CMD(
     ...
     "Sample: pwm 0 100000 200000 1 14\n", pwm_help_text
 );
#endif
...
extern int do_bootm_core1(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[],
            int states, bootm_headers_t *images, int boot_progress);
int do_bootmc1(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    int ret;

    argc--; argv++;

    ret = do_bootm_core1(cmdtp, flag, argc, argv, BOOTM_STATE_START |
...
U_BOOT_CMD(
    bootmc1,	CONFIG_SYS_MAXARGS,	1,	do_bootmc1,
    "boot application image with core1", "[addr [arg ...]]\n    - boot application image stored in memory\n"
);
#endif

#if defined(CONFIG_SSTAR_RIU_FILE)
static char riu_help_text[] =
     "addr len: load the riu configure file to set the reg\n"
     "the file need to be load into dram at [addr] with [len]\n"
     "the file format: (1)wriu -w [reg] [value],(2)wait [value],(3)// read addr : [reg] , check data : [value]\n";

int do_riu_file(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    int read_value;
    int delay;
    int reg_command_count;
    int wait_command_count;
    int read_command_count;

    switch(argc)
    {
    case 1:
    case 2:
        printf("[Error]:using riu without complete parameter!\r\n");
        return -1;
    case 3:
    ...
    reg_command_count = 0;
    wait_command_count = 0;
    read_command_count = 0;
    printf("command list:\n");
    while( token != NULL )
    {
        ret = strstr(token,"wriu");
        if(ret)
        {
            reg_command_count++;
            sscanf(token,"%*s %*s %x %x",&reg_addr,&reg_value);
            printf("write to reg:0x%x, with value:0x%x\n",reg_addr,reg_value);
            Write2Byte(reg_addr,reg_value);
        }

        ret = strstr(token,"wait");
        if(ret)
        {
            wait_command_count++;
            sscanf(token,"%*s %d",&delay);
            printf("wait with delay:%d\n",delay);
            udelay(delay);
        }

        ret = strstr(token,"read");
        if(ret)
        {
            read_command_count++;
            sscanf(token,"%*s %*s %*s %*s %d %*s %*s %*s %*s %d",&reg_addr,&reg_value);
            printf("read  to reg:0x%x, with value:0x%x\n",reg_addr,reg_value);
            read_value = Read2Byte(reg_addr);
            if(read_value != reg_value)
            {
                printf("the reg:0x%x value:0x%x is not expected!!!\n",reg_addr,read_value);
            }
        }
    ...
    printf("command end,with reg command %d,wait command %d,read reg command %d\n",reg_command_count,wait_command_count,read_command_count);
    free(buffer);
...
U_BOOT_CMD(
     riuf,  CONFIG_SYS_MAXARGS,    1,    do_riu_file,
     "load riu file test for sstar\n",
     riu_help_text
 );
